...
namespace Game.Character.Movement.Locomotion.Installers
{
    public class CharacterLocomotionInstaller : MonoInstaller
    {
        ...
        {
            Container
                .BindInstance(_characterLocomotionModel).AsSingle();
            
            Container
                .BindInterfacesTo<CharacterLocomotionController>().AsSingle();

            Container.BindInstance(_characterController)
                .AsSingle();
            
            Container.BindInstance(_jumpingAnimationParamName)
                .WithId(BindingIdentifiers.JumpParamId);
            
            Container.BindInstance(_sprintingAnimationParamName)
                .WithId(BindingIdentifiers.SprintParamId);
        }
    }
}
